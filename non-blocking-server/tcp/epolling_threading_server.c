...
void *worker(void *arg) {
    struct worker_info winfo = *(struct worker_info *) arg;
    struct epoll_event ev, events[MAXEVENTS];
    while (atomic_keep_running) {
        int nfds = epoll_wait(winfo.epfd, events, MAXEVENTS, -1);
        if (nfds == -1) {
            perror("epoll_wait");
            break;
        }
        for (int i = 0; i < nfds; i++) {
            if (events[i].data.fd == winfo.sockfd) {
                while (atomic_keep_running) {
                    int cltsock = accept_connection(winfo.sockfd);
                    if (cltsock == -1) {
                    ...
                    ev.data.fd = cltsock;
                    epoll_ctl(winfo.epfd, EPOLL_CTL_ADD, cltsock, &ev);
                }
            }
            else {
...
void serve(int srvsock) {
    ...
    for (int i = 0; i < NUMWORKERS; i++) {
        ...
        winfo.epfd = epfd;
        winfo.sockfd = srvsock;
        int res = pthread_create(&threads[i], NULL, worker, &winfo);
        if (res != 0) {
